# FavorMe — AI 心理洞察助手（Flutter Android MVP）

## What This Is

FavorMe 是一款面向日常「小而即时纠结」的 **AI 心理洞察助手**：不替用户下结论，而是通过 **三道带选项的结构化问题** 引导用户自我探索，最后给出 **倾向性反馈**（帮助用户看见内心约 `51%` 的那一侧）。当前 GSD 工作重心是 **`clients/flutter` 中的 Flutter-first Android 客户端 + 自建后端** 的最小闭环，与仓库内 `docs/product-scope.md` 的产品口径一致。
...
- [ ] **Android 客户端**：在 `clients/flutter` 中交付 Flutter-first 的基础「对话式」窗口（输入问题、展示服务端返回的三问与选项、展示结论），可先不做完整账号体系
...
- **iOS 首发、小程序、纯 Web 首发** — 试水期策略为 Android 优先（见 `docs/product-scope.md`）；其他形态待验证后排期
...
- **Brownfield**：仓库已有 `demo/web`、`backend`、Prisma 模型与 `docs/modules/ai-chat-orchestration.md` 等；Phase 2 Android 主客户端路径已锁定为 `clients/flutter`，`clients/android` 只保留平台说明入口。
...
- **客户端**：试水期 **Flutter-first Android-only**，主实现位于 `clients/flutter`；`clients/android` 不承载 Phase 2 MVP 功能代码。
...
|----------|-----------|---------|
| Flutter-first Android 作为 MVP 主端 | Phase 2 视觉与后续 iOS 复用需要统一 Flutter UI；当前仅验收 Android | ✓ Good |
| 三问问卷式交互，非长对话 | 产品差异化与轻量化 | — Pending |
| 后端统一 LLM 与规则 | 安全与 prompt 可控 | — Pending |
| Brownfield 先 map codebase | GSD 流程要求 | ✓ Good |
| `clients/android` 作为说明入口 | 避免 Phase 2 代码分散到两个客户端目录 | ✓ Good |
